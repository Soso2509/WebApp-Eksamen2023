import type { User, QueryParams, Result, WithPagination } from "@/types"
import type { NextRequest } from "next/server"
import { NextResponse } from "next/server"

import * as userService from "./product.service"
...
export const listProducts = async (
  ...
  const products = await userService.list(params)
  if (!products.success)
...
export const createProduct = async (
  ...
  const createdProduct = await userService.create({
    price,
    name,
    sku,
    status,
...
export const getProductById = async (
  ...
  const product = await userService.getById({
    id,
  })
...
export const updateProductById = async (
  ...
  const product = await userService.putById(id, productData)
...
export const publishProductById = async (
  ...
  const product = await userService.putById(id, { status: "published" })
...
export const removeProductById = async (
  ...
  const removedProduct = await userService.deleteById({
    id,
  })
